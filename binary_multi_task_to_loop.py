...
#from generate_data_set_multi_task import *

from generate_data_set_multi_task_3_tasks import *
...
class EarlyStoppingByLossVal(Callback):
    def __init__(self, monitor='val_loss', value=0.00009, verbose=0):
...
def and_fun(t,N_rec,base,base_plot):
    ...
    sample_size      =3*15050#15050#8242 # (2^11 samples +50 for testing its a good value 2^13 +50 is better)
    epochs           = 20
    #N_rec            = 50 #100
    p_connect        = 0.9
    ...
    model = Sequential()    
    model.add(SimpleRNN(units=N_rec,return_sequences=True, input_shape=(None, 3), kernel_initializer='glorot_uniform', recurrent_initializer='orthogonal', activation='tanh',use_bias=False)) #defaults for the recurrent model!
    #model.add(LSTM(units=N_rec,return_sequences=True, input_shape=(None, 3), kernel_initializer='glorot_uniform', recurrent_initializer='orthogonal', activation='tanh',use_bias=True))
    #model.add(GRU(units=N_rec,return_sequences=True, input_shape=(None, 3), kernel_initializer='glorot_uniform', recurrent_initializer='orthogonal', activation='tanh',use_bias=True))
    ...
    for ii in np.arange(10):
        ...
        a=lista_distancia[ii]
        a="%.2f" % a
        plt.plot(x_train[ii, :, 0],color='g',label="Input A")
        plt.plot(x_train[ii, :, 1],color='pink',label="Input B")
        plt.plot(x_train[ii, :, 2],linewidth=1,color='deepskyblue',label='Context Input')
        plt.plot(y_train[ii, 0:-2, 0],color='gray',linewidth=1,label="Expected Output")
